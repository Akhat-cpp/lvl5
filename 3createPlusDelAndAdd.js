let dataTable = {};//алгоритм заполнения, идем по айди, если есть айди 1 не занятый то берем его, если есть 2 не занятый 
...
function createBody(data, id)
{
	...
	while(counter1 != data.length)
	{
		...
		while(counter != dataTable[id + '.size']+1)
		{
			...
			{
				table += '<td><button onclick="deleteEl(`' + data[counter1].id + '`, `' + id + '`)">' + 'Del' +'</button></td>';				
			}
			else
			{
